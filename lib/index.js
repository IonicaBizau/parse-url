...
function parseUrl(url, normalize = false) {
    ...
    const js_check = (!url.startsWith('//') && url.substring(0,url.indexOf(':')).indexOf('javascript') !== -1 ? javascript_check(url) : false)
    const js_inv_check = (js_check==true?((url.substring(url.indexOf(':'), url.indexOf(':') + 3) == '://') == js_check):true)

    if (js_check){
        url = ((url.substring(0, url.indexOf(':')) == 'javascript' && js_inv_check) || js_inv_check
    ...
    if(js_inv_check !== true){
        parsed['href'] = [url.slice(0, url.indexOf(':///')+1), url.slice(url.indexOf(':///')+4)].join('')
    }
    return parsed
}
